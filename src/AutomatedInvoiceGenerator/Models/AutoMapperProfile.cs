...
namespace AutomatedInvoiceGenerator.Models
{
    public class AutoMapperProfile : Profile
    {
        public AutoMapperProfile()
        {
            ...
            CreateMap<OneTimeServiceItem, OneTimeServiceItemDto>().ForMember(dest => dest.InvoiceItemShorts, opt => opt.ResolveUsing(s => s.InvoiceItems)).ReverseMap().ForMember(dest => dest.InvoiceItems, opt => opt.Ignore());
            CreateMap<SubscriptionServiceItem, SubscriptionServiceItemDto>().ForMember(dest => dest.InvoiceItemShorts, opt => opt.ResolveUsing(s => s.InvoiceItems)).ReverseMap().ForMember(dest => dest.InvoiceItems, opt => opt.Ignore());
